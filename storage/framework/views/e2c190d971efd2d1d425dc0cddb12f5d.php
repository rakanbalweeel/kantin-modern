    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="relative overflow-hidden bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-500 rounded-3xl shadow-2xl p-8 text-white mb-8"
            ...
            <div x-show="showTopup" 
                ...
                <form action="<?php echo e(route('siswa.saldo.topup')); ?>" method="POST" class="space-y-4">
                    <?php echo csrf_field(); ?>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/50 overflow-hidden">
            ...
            <div class="divide-y divide-gray-100">
                <?php $__empty_1 = true; $__currentLoopData = $riwayatTopup; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $topup): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="px-6 py-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                        <div class="flex items-center">
                            <div class="w-10 h-10 rounded-full flex items-center justify-center mr-4
                                <?php if($topup->status === 'pending'): ?> bg-amber-100 text-amber-600
                                <?php elseif($topup->status === 'approved'): ?> bg-green-100 text-green-600
                                <?php else: ?> bg-red-100 text-red-600 <?php endif; ?>">
                                <i class="fas 
                                    <?php if($topup->status === 'pending'): ?> fa-clock
                                    <?php elseif($topup->status === 'approved'): ?> fa-check
                                    <?php else: ?> fa-times <?php endif; ?>"></i>
                            </div>
                            <div>
                                <p class="font-semibold text-gray-800">Rp <?php echo e(number_format($topup->jumlah, 0, ',', '.')); ?></p>
                                <p class="text-sm text-gray-500"><?php echo e(\Carbon\Carbon::parse($topup->created_at)->format('d M Y, H:i')); ?></p>
                            </div>
                        </div>
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold
                            <?php if($topup->status === 'pending'): ?> bg-amber-100 text-amber-700
                            <?php elseif($topup->status === 'approved'): ?> bg-green-100 text-green-700
