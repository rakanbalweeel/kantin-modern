...
<?php $__env->startSection('title', 'Daftar Pesanan - Kantin'); ?>
...
<div class="min-h-screen hero-gradient py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="glass-card rounded-2xl p-6 mb-8">
            <form action="<?php echo e(route('kantin.orders.index')); ?>" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-300 mb-2">Cari</label>
                    <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Kode pesanan / Nama siswa" 
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-4 py-2.5 rounded-xl bg-slate-800/50 border border-slate-700 text-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition-all">
                        ...
                        <option value="diproses" <?php echo e(request('status') == 'diproses' ? 'selected' : ''); ?> class="bg-slate-800">Diproses</option>
                        <option value="selesai" <?php echo e(request('status') == 'selesai' ? 'selected' : ''); ?> class="bg-slate-800">Selesai</option>
                        <option value="batal" <?php echo e(request('status') == 'batal' ? 'selected' : ''); ?> class="bg-slate-800">Batal</option>
                    </select>
                </div>
                <div>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="<?php echo e(route('kantin.orders.index')); ?>" class="px-4 py-2.5 bg-slate-700 text-slate-300 rounded-xl hover:bg-slate-600 transition-all">
                        <i class="fas fa-times"></i>
                    </a>
                </div>
            </form>
                    ...
                    <tbody class="divide-y divide-slate-700/50">
                        <?php $__empty_1 = true; $__currentLoopData = $orders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-white/5 transition-colors">
                                <td class="px-6 py-4">
                                    <span class="font-bold text-white font-mono"><?php echo e($order->kode_pesanan); ?></span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="w-10 h-10 bg-gradient-to-br from-orange-500 to-amber-500 rounded-xl flex items-center justify-center mr-3">
                                            <span class="text-white font-bold text-sm"><?php echo e(strtoupper(substr($order->user->name, 0, 1))); ?></span>
                                        </div>
                                        <div>
                                            <p class="font-medium text-white"><?php echo e($order->user->name); ?></p>
                                            <p class="text-xs text-slate-500"><?php echo e($order->user->email); ?></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-slate-400"><?php echo e($order->orderDetails->sum('jumlah')); ?> item</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="font-bold text-orange-400">Rp <?php echo e(number_format($order->total, 0, ',', '.')); ?></span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-slate-300"><?php echo e($order->created_at->format('d/m/Y')); ?></div>
                                    <div class="text-xs text-slate-500"><?php echo e($order->created_at->format('H:i')); ?></div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="<?php echo e(route('kantin.orders.show', $order)); ?>" class="p-2 bg-slate-700 text-slate-300 rounded-lg hover:bg-orange-500/20 hover:text-orange-400 transition-all" title="Detail">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        
                                        <?php if($order->status === 'pending'): ?>
                                            <form action="<?php echo e(route('kantin.orders.accept', $order)); ?>" method="POST" class="inline">
                                                <?php echo csrf_field(); ?>
                                        ...
                                        <?php if($order->status === 'diproses'): ?>
                                            <form action="<?php echo e(route('kantin.orders.complete', $order)); ?>" method="POST" class="inline">
                                                <?php echo csrf_field(); ?>
                                                <?php echo method_field('PATCH'); ?>
                                        ...
                                        <?php if(in_array($order->status, ['pending', 'diproses'])): ?>
                                            <form action="<?php echo e(route('kantin.orders.cancel', $order)); ?>" method="POST" class="inline" onsubmit="return confirm('Yakin ingin membatalkan pesanan ini?')">
                                                <?php echo csrf_field(); ?>
                                                <?php echo method_field('PATCH'); ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\kantin-modern\resources\views/kantin/orders/index.blade.php ENDPATH**/ ?>