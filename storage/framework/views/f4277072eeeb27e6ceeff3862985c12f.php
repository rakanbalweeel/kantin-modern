...
<?php $__env->startSection('title', 'Kelola Penarikan - Admin'); ?>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                ...
                <a href="<?php echo e(route('admin.dashboard')); ?>" class="inline-flex items-center px-4 py-2 glass rounded-xl text-slate-300 hover:text-orange-400 transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            ...
            <div class="relative rounded-2xl p-6 overflow-hidden bg-gradient-to-br from-yellow-500/20 to-amber-500/20 border border-yellow-500/30">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <span class="text-3xl font-extrabold text-yellow-400"><?php echo e($stats['pending_count']); ?></span>
                </div>
                <p class="text-sm font-medium text-yellow-300/80 mb-1">Menunggu Approval</p>
                <p class="text-xl font-bold text-yellow-400">Rp <?php echo e(number_format($stats['pending'], 0, ',', '.')); ?></p>
            </div>
            ...
            <div class="glass-card rounded-2xl p-6 hover:bg-white/5 transition-all duration-300">
                ...
                <p class="text-2xl font-extrabold text-emerald-400">
                    Rp <?php echo e(number_format($stats['approved_this_month'], 0, ',', '.')); ?>
            ...
            <div class="glass-card rounded-2xl p-6 hover:bg-white/5 transition-all duration-300">
                ...
                <p class="text-2xl font-extrabold text-blue-400">
                    Rp <?php echo e(number_format($stats['total_pajak_collected'], 0, ',', '.')); ?>
            ...
            <div class="glass-card rounded-2xl p-6 hover:bg-white/5 transition-all duration-300">
                ...
                <p class="text-2xl font-extrabold text-purple-400">
                    <?php echo e(\App\Models\Setting::get('pajak_withdrawal', 5)); ?>%
                </p>
            </div>
        </div>
            ...
            <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <select name="status" onchange="this.form.submit()" class="w-full px-4 py-3 rounded-xl bg-slate-800/50 border border-slate-700 text-white focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 transition-all">
                        ...
                        <option value="pending" <?php echo e(request('status') === 'pending' ? 'selected' : ''); ?> class="bg-slate-800">Pending</option>
                        <option value="approved" <?php echo e(request('status') === 'approved' ? 'selected' : ''); ?> class="bg-slate-800">Disetujui</option>
                        <option value="rejected" <?php echo e(request('status') === 'rejected' ? 'selected' : ''); ?> class="bg-slate-800">Ditolak</option>
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Kode / Nama Kantin"
                ...
                <div class="flex items-end gap-2">
                    ...
                    <?php if(request()->hasAny(['status', 'date', 'search'])): ?>
                        <a href="<?php echo e(route('admin.withdrawals.index')); ?>" class="px-4 py-3 bg-slate-700 text-slate-300 rounded-xl hover:bg-slate-600 transition-colors">
                            <i class="fas fa-times"></i>
                        </a>
                    <?php endif; ?>
                </div>
                    ...
                    <tbody class="divide-y divide-slate-700/50">
                        <?php $__empty_1 = true; $__currentLoopData = $withdrawals; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $withdrawal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="hover:bg-white/5 transition-colors" x-data="{ showReject: false }">
                                <td class="px-6 py-4">
                                    <span class="font-mono font-semibold text-white"><?php echo e($withdrawal->kode_withdrawal); ?></span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        ...
                                        <span class="font-medium text-slate-300"><?php echo e($withdrawal->user->name); ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="font-semibold text-white">Rp <?php echo e(number_format($withdrawal->jumlah, 0, ',', '.')); ?></span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-red-400 font-medium">
                                        - Rp <?php echo e(number_format($withdrawal->pajak_nominal, 0, ',', '.')); ?>

                                        <span class="text-xs text-slate-500">(<?php echo e($withdrawal->pajak_persen); ?>%)</span>
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="font-bold text-emerald-400">Rp <?php echo e(number_format($withdrawal->jumlah_bersih, 0, ',', '.')); ?></span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    ?>
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium <?php echo e($config['bg']); ?> <?php echo e($config['text']); ?> border <?php echo e($config['border']); ?>">
                                    ...
                                    <?php if($withdrawal->status === 'pending'): ?>
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <form action="<?php echo e(route('admin.withdrawals.approve', $withdrawal)); ?>" method="POST" class="inline">
                                                ...
                                                <button type="submit" onclick="return confirm('Setujui penarikan ini? Pajak Rp <?php echo e(number_format($withdrawal->pajak_nominal, 0, ',', '.')); ?> akan masuk ke kas admin.')"
                                        ...
                                        <div x-show="showReject" x-cloak
                                            ...
                                            <div class="glass-card rounded-2xl w-full max-w-md p-6 m-4" @click.stop>
                                                ...
                                                <form action="<?php echo e(route('admin.withdrawals.reject', $withdrawal)); ?>" method="POST">
                                                    <?php echo csrf_field(); ?>
                                                    <?php echo method_field('PATCH'); ?>
                                                    <div class="mb-4">
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\laragon\www\kantin-modern\resources\views/admin/withdrawals/index.blade.php ENDPATH**/ ?>