    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-10">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                ...
                <a href="{{ route('admin.dashboard') }}" class="mt-4 lg:mt-0 inline-flex items-center px-4 py-2 glass rounded-xl text-slate-300 hover:text-orange-400 transition-colors">
        ...
        <form action="{{ route('admin.settings.pajak') }}" method="POST" x-data="{ 
            selectedPajak: {{ $pajak_persen }}, 
            selectedPajakWithdrawal: {{ $pajak_withdrawal }},
            loading: false 
        }" @submit="loading = true">
            @csrf
                    ...
                    <div class="mb-4 p-4 bg-amber-500/10 rounded-xl border border-amber-500/30">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-3xl font-extrabold text-amber-400">{{ number_format($pajak_persen, 1) }}%</p>
                            </div>
                            <div class="text-right text-sm">
                                <p class="text-slate-500">Contoh: Rp 100.000</p>
                                <p class="text-amber-400">Pajak: Rp {{ number_format(100000 * $pajak_persen / 100, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="glass-card rounded-2xl p-6">
                    ...
                    <div class="mb-4 p-4 bg-purple-500/10 rounded-xl border border-purple-500/30">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-xs text-slate-400 mb-1">Pajak Saat Ini</p>
                                <p class="text-3xl font-extrabold text-purple-400">{{ number_format($pajak_withdrawal, 1) }}%</p>
                            </div>
                            <div class="text-right text-sm">
                                <p class="text-slate-500">Contoh: Rp 100.000</p>
                                <p class="text-purple-400">Pajak: Rp {{ number_format(100000 * $pajak_withdrawal / 100, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
