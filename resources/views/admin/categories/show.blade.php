{{--
==========================================================================
DETAIL KATEGORI (SHOW) - DARK THEME
==========================================================================
--}}
        ...
        <div class="glass-card rounded-2xl p-6 mb-8">
            <div class="flex items-start justify-between">
                <div class="flex items-center">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-white">{{ $category->nama }}</h1>
                        <p class="text-slate-400">{{ $category->deskripsi ?: 'Tidak ada deskripsi' }}</p>
                    </div>
                </div>
            ...
            <div class="mt-6 grid grid-cols-2 gap-4">
                <div class="bg-slate-800/50 rounded-xl p-4">
                    ...
                    <p class="text-2xl font-bold text-orange-400">{{ $category->products->count() }}</p>
                </div>
                <div class="bg-slate-800/50 rounded-xl p-4">
                    <p class="text-sm text-slate-400 mb-1">ID Kategori</p>
                    <p class="text-2xl font-bold text-white">{{ $category->id }}</p>
                </div>
            </div>
        </div>
                ...
                <div class="divide-y divide-slate-700/50">
                    @foreach($category->products as $product)
                        <div class="p-4 flex items-center justify-between hover:bg-white/5 transition-colors">
                            <div class="flex items-center">
                                @if($product->gambar)
                                    <img src="{{ asset('storage/' . $product->gambar) }}" alt="{{ $product->nama }}" class="w-12 h-12 rounded-xl object-cover mr-4">
                                @else
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-slate-400">Stok: {{ $product->stok }}</p>
                                </div>
                            </div>
                            <div class="text-right">
                                <p class="font-bold text-orange-400">Rp {{ number_format($product->harga, 0, ',', '.') }}</p>
                                <a href="{{ route('admin.products.show', $product) }}" class="text-sm text-slate-400 hover:text-orange-400">
                                    Lihat Detail →
                                </a>
                            </div>
                        </div>
