{{--
==========================================================================
KANTIN - DETAIL PESANAN (DARK THEME)
==========================================================================
--}}

@extends('layouts.app')

@section('title', 'Detail Pesanan - Kantin')
...
<div class="min-h-screen hero-gradient py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {{-- Header --}}
        <div class="mb-8">
            <a href="{{ route('kantin.orders.index') }}" class="inline-flex items-center text-slate-400 hover:text-orange-400 transition-colors mb-4">
            ...
            <div class="glass-card rounded-2xl p-6 mb-8" x-data="{ showCancelModal: false, loading: false }">
                ...
                <div class="flex flex-wrap gap-4">
                    @if($order->status === 'pending')
                        <form action="{{ route('kantin.orders.accept', $order) }}" method="POST" x-data="{ submitting: false }" @submit="submitting = true">
                            @csrf
                            @method('PATCH')
                    ...
                    @if($order->status === 'diproses')
                        <form action="{{ route('kantin.orders.complete', $order) }}" method="POST" x-data="{ submitting: false }" @submit="submitting = true">
                            @csrf
                            @method('PATCH')
                ...
                <div x-show="showCancelModal" 
                    ...
                    <div class="glass-card rounded-2xl max-w-md w-full p-6" 
                        ...
                        <div class="text-center mb-6">
                            ...
                            <p class="text-slate-400">Pesanan <strong class="text-white">{{ $order->kode_pesanan }}</strong> akan dibatalkan dan stok produk akan dikembalikan.</p>
                        </div>
                        <div class="flex gap-3">
                            ...
                            <form action="{{ route('kantin.orders.cancel', $order) }}" method="POST" class="flex-1" x-data="{ submitting: false }" @submit="submitting = true">
                                @csrf
                                @method('PATCH')
            ...
            <div class="glass-card rounded-2xl p-6">
                ...
                <div class="space-y-3">
                    <div class="flex justify-between">
                        ...
                        <span class="text-white font-medium">{{ $order->created_at->locale('id')->isoFormat('D MMM Y') }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-400">Waktu Pesan</span>
                        <span class="text-white font-medium">{{ $order->created_at->format('H:i') }} WIB</span>
                    </div>
                    ...
                    @if($order->updated_at != $order->created_at)
                        <div class="flex justify-between">
                            ...
                            <span class="text-white font-medium">{{ $order->updated_at->locale('id')->isoFormat('D MMM Y, HH:mm') }}</span>
                        </div>
                    @endif
                </div>
            </div>
                ...
                <div class="space-y-6">
                    <div class="relative flex items-start pl-10">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-400">{{ $order->created_at->locale('id')->isoFormat('D MMM Y, HH:mm') }}</p>
                        </div>
                    </div>
                    ...
                    @if($order->status === 'selesai')
                        <div class="relative flex items-start pl-10">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-slate-400">{{ $order->updated_at->locale('id')->isoFormat('D MMM Y, HH:mm') }}</p>
                            </div>
                        </div>
                    @endif
